...
Class HTMLWindow
	...
	Private fLogTick
    Private m_Cuerpo,m_Log
    Private m_CurrContainer   ' <-- Contenedor abierto (div interno)
	Private oDicContainerStack
	...
	Public Function CreaVentanaHTML (strTitulo, Visible, Width, Height)
        ...
        If IsEmpty (Width) Or IsEmpty (Height) Then
			...
			For Each objItem in colItems
			    If IsEmpty (Width) Then objExplorer.Width = objItem.ScreenWidth - 20
			    If IsEmpty (Height) Then objExplorer.Height = objItem.ScreenHeight - 60
			Next
		End If
	...
	Public Function Init_Document(strTitulo)
		...
		' de momento essta instruccion debe estar despues de crear el m_Log
	...
	Public Property Set oCurrContainer (oContainer)
		' me aseguro de que oContainer NO sea m_CurrContainer, si no SALGO!
		If oContainer Is m_CurrContainer Then
			...
			Exit Property
		End If
		' push m_CurrContainer to stack
		' me aseguro de que en la stack NO este el m_CurrContainer, si no NO lo meto (... y NO debería estar, porque ...
		Dim lastContainer
		...
		If m_CurrContainer Is Nothing And oDicContainerStack.Count = 0 Then ' 
			' ESTO siempre pasa cuando se inicializa CreaVentanaHTML
		ElseIf (m_CurrContainer Is lastContainer) Or (m_CurrContainer Is Nothing) Then
			' esto parece que NO deberia pasar, ¿no, ChatGPT?, parece que la condicion de salida anterior, If oContainer Is m_CurrContainer Then..., lo impide...
			' o es "una cagada" mia, y estoy asignando Nothing a oCurrContainer ;-)
			Stop
		Else
	...
	Public Function existsContainer (strSpoilerID)
		existsContainer = Not IsNull(objDocument.getElementByID(strSpoilerID))
	End Function
		
	Public Function setContainer (strSpoilerID)
		' Obtiene el Container correspondiente al ID
		Select Case True
			Case IsEmpty (strSpoilerID)
				Set setContainer = m_Cuerpo
			Case IsNull(objDocument.getElementByID(strSpoilerID))
		...
		End Select
		' Si se ha conseguido un Container, lo establece como actual
	...
	Public Function Spoiler (bOpen,ByVal style,linea_texto, ByVal strSpoilerID,bNoWrap)
        ...
        If Not IsNull(objDocument.getElementByID(strSpoilerID)) Then
        ...
        If bOpen Then
            ...
            Select Case True
            	Case m_CurrContainer Is Nothing, IsNull(m_CurrContainer.getAttribute("ID"))
            	Case Else
	...
	Public Function closeSpoiler ()
		' resuelve si el cierre de contexto es contra el "parent", o el m_Cuerpo
        Select Case True
        	Case IsEmpty(m_CurrContainer),m_CurrContainer Is Nothing, IsNull(objDocument.getElementByID(m_CurrContainer.getAttribute("data-parent"))), _
	...
	Public Default Sub MsgIE (ByVal strout)
		' escribe un mensaje EN EL CONTENEDOR ACTUAL
		If objExplorer Is Nothing Then
			Err.Raise 91, "HTMLWindow", "InternetExplorer.Application not initialized. Call CreaVentanaHTML() first."
		End If
	...
	Public Sub MsgLog (linea_texto)
		...
		'fLogTick = Timer
		' Usar scrollHeight - clientHeight evita el recorte de la última línea
		m_Log.scrollTop = m_Log.scrollHeight  - m_Log.clientHeight ' auto-scroll hacia abajo
	    ' Asegura que la última línea quede visible (alineada abajo):
