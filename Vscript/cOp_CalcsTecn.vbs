...
Class cOp_CalcsTecn
	...
	Function Init (strFold, ExcelApplication)
		Set Init = Me
		Me.strfold = strfold
	...
	Function identificaCalcTecnicos ()
		...
		Dim bMoveFile,strtmpfold,strFich,strFilter
				...
				If strFich <> "" then
					...
					If regex.Test (fich.Name) And Left (fich.Name,2) <> "~$" Then
						...
						If bMoveFile Then
							...
							If m_ExcelApp.RenameWorkbook(fich.Path, strFold & "\" & fich.Name) Then
								Set fich = fso.getFile (strFold & "\" & fich.Name)
							End if
...
'				 & "</ul></font>"
		...
		For Each strFPath In oDicCalcs(strCalcNum)(strCalcOpc).Keys
			...
			MsgIE "Procesando fichero " & strFName 
			...
			Select Case True
				...
				Case match.Submatches(3) = "_calc"
					...
					Else
						...
						If Not oABCGas_XLS is Nothing Then
							'Stop
