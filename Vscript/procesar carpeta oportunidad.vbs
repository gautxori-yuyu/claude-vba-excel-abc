...
Dim MsgIE
Set MsgIE = (New HTMLWindow).CreaVentanaHTML ("procesado oportunidades", True, Empty, Empty)', 850, 1200)
...
' =============================================
Dim arg,oOportunidad
For Each arg In arrFolds
	If bQuit Then WScript.Quit
	If fso.FolderExists (arg) Then
		MsgIE.objDocument.title = Replace (MsgIE.objDocument.title,"procesado oportunidades",fso.GetFolder(arg).Name & " / " & "procesado oportunidades")
		Set oOportunidad = (New cOportunidad).Init(arg, ExcelApp(), MsgIE)
        Call oOportunidad.procesaCarp()
		WshShell.Popup "Finalizado el procesado de la oportunidad: " & fso.GetFolder(arg).Name, 20
