...
Class cABCGas_XLS
	...
	Private Sub Class_Initialize()
		...
		Set oDicGasComp = WScript.CreateObject("scripting.dictionary")
		Set oDicStages = WScript.CreateObject("scripting.dictionary")
	...
	Public Function Init(ExcelApp, strXLSXPath_)
		...
		If oCell_Compressor_Serie = "HG" Then
			If bH2 Then MsgBox ("Muy posiblemente el compresor es API-618 (H2), CONVIENE HACERLO EN PLATAFORMA HP")
...
'		    next
	...
	Function ValidarCalculos
		...
		MsgIE ("<u>VALIDACION DE CONDICIONES API-618</u>")
		' si se solicita expresamente que el compresor sea API 618, de manda a ADVERTENCIAS; si no, "en azul"
		bAPI618 = MsgBox ("El compresor DEBE ser API 618 por requerimiento de cliente?") = 6
		If Not bAPI618 Then
			'SEGUIR AQUI
			...
			MsgIE "<font color=blue>El calculo ""RATED"" se hace PARA LAS TEMPERATURAS MAS ALTAS, LUEGO:" & vbCrLf & _
					...
					"<li> la TEMPERATURA DE SALIDA DEL GAS, SIEMPRE la podemos dar 10 POR ENCIMA DE LA DEL AGUA DE REFRIGERACION, usando un INTERCOOLER FINAL (pero SIEMPRE hay que respetar los lims de temperatura del gas!)" & "</li>" _
					 & "</ul></font>"
		End If
		MsgBox ("VALIDACION DE CONDICIONES API-618:")
		If oCell_Compressor_Serie = "HG" Then MsgBox ("- **** LA PLATAFORMA HG NO ESTÁ PENSADA PARA SER 'TODO API-618', SERIA CONVENIENTE HACER EL COMPRESOR EN HP, que SI está pensada para API ****")
		If CellValue(oCell_OUTProcess_PistonMeanSpeed) > 3.5 Then MsgBox ("- OJO!!: la velocidad media del pistón, " & oCell_OUTProcess_PistonMeanSpeed & ", es DEMASIADO ELEVADA, SUPERA EL LIMITE DE 3.5 m/s. Sería conveniente REDUCIR LAS RPMs, AUMENTANDO DIAMETROS, ...") 
		MsgBox ("- Asegurarse de que EN TODAS LAS ETAPAS de ABCAire, LOS CILINDROS DEBEN IR ENCAMISADOS!!! (tenderá a AUMENTAR LAS RPM!!!)")
		MsgBox ("- Superbolt, y recubrimiento de Vastagos en WC, serían opciones tb a incluir, en OFERGAS, en ""OTROS""!!")
		MsgBox ("OTRAS NORMAS a tener en cuenta, que NO afectan a ABC Aire: TEMA, en refrigeradores; y ASME, o Merkblatter, en calderines antipulsadores. Y para compresores DE AIRE, en Petroquimica, API 617 y API 672")
		...
		If bBombaAceiteAux Then MsgBox ("NOTA: este modelo de compresor, " & strModelName & ", LLEVA POR DEFECTO UNA BOMBA DE ACEITE AUXILIAR (""redundante""), ACCIONADA POR MOTOR ELECTRICO en el sist de lubricación. CON ELLO SE CUMPLE API 610 y la norma ISO 10438-3. DEBERIA considerarse en los precios por defecto (OFERGAS), pero EN EL BUDGET / QUOTATION, posiblemente haya que indicarlo!!")
		If bBielasForjadas Then MsgBox ("NOTA: este modelo de compresor, " & strModelName & ", LLEVA POR DEFECTO las bielas FORJADAS (EN EL BUDGET / QUOTATION, posiblemente haya que indicarlo)")
		If iStroke <> 0 Then MsgBox ("NOTA: POR DEFECTO, este modelo de compresor, " & strModelName & ", tiene una CARRERA MAXIMA DEL PISTÓN, DE " & iStroke & " MM ** pero se podría haber modificado en ABC ** (EN EL BUDGET / QUOTATION, posiblemente haya que indicarlo)")
		If iRodDiam <> 0 Then MsgBox ("NOTA: POR DEFECTO, este modelo de compresor, " & strModelName & ", tiene un diametro de vastago, DE " & iRodDiam & " MM ** pero se podría haber modificado en ABC ** (EN EL BUDGET / QUOTATION, posiblemente haya que indicarlo)")
		' capacidad en litros del CARTER, interesante para los calculos de MTTO 8000 h:
		If iCrankCaseLtr <> 0 Then MsgBox ("NOTA: este modelo de compresor, " & strModelName & ", tiene un CARTER DE " & iCrankCaseLtr & " L (UTIL PARA CALCULAR LITROS DE ACEITE, EN EL INFORME DE MTTO 8000 H)")
		If bAir Then MsgBox ("NOTA: este modelo de compresor, " & strModelName & ", por ser de Aire, NO LLEVA LLAVE DE ENTRADA")
