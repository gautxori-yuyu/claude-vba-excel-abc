...
Private Function ObtenerFactorCorreccion(c As Range) As Double
    ...
    Select Case True
        ...
        Case Else
            LogInfo MODULE_NAME, "[ObtenerFactorCorreccion] Metadatos y datos ocultos eliminados correctamente."
            LogInfo MODULE_NAME, "[ObtenerFactorCorreccion] Fuente: " & c.Font.Name & " - " & c.Font.SIZE
            Stop
    End Select
    ...
    If c.Interior.ColorIndex <> xlNone Then
        Select Case c.Interior.Color
            ...
            Case Else
                LogInfo MODULE_NAME, "[ObtenerFactorCorreccion] Color de fondo: " & LongToRGB(c.Interior.Color)
...
Attribute AjustarSelWSheetsParaImpresionPDF.VB_ProcData.VB_Invoke_Func = " \n0"
    ...
    LogInfo MODULE_NAME, "[AjustarSelWSheetsParaImpresionPDF] " & selSheets.Count & " hoja(s) en '" & Wb.Name & "' ==="
    ...
    For Each Sh In selSheets
        ...
        If TypeName(Sh) = "Worksheet" Then
            ...
            Else
                LogError MODULE_NAME, "[AjustarSelWSheetsParaImpresionPDF] Falló en '" & Sh.Name & "': " & Err.Description
                hojasFallidas = hojasFallidas + 1
            End If
            On Error GoTo 0
        Else
            LogInfo MODULE_NAME, "[AjustarSelWSheetsParaImpresionPDF] Saltada hoja no Worksheet: '" & Sh.Name & "' (tipo: " & TypeName(Sh) & ")"
        End If
    Next Sh
    
    LogInfo MODULE_NAME, "[AjustarSelWSheetsParaImpresionPDF] Proceso finalizado: " & hojasProcesadas & " hojas ajustadas, " & hojasFallidas & " con error."
...
CleanUp:
    ...
    LogInfo MODULE_NAME, "[AjustarSelWSheetsParaImpresionPDF] Restauración de las áreas de impresión de cada hoja del libro:"
    hojasProcesadas = 0
    hojasFallidas = 0
    i = 0
    For Each Sh In selSheets
...
ErrorHandler:
    If Err.Number <> 0 Then
        LogCurrentError MODULE_NAME, "[AjustarSelWSheetsParaImpresionPDF]"
...
CleanUp:
    ...
    If Err.Number <> 0 Then
        LogCurrentError MODULE_NAME, "[AjustarWSParaPDF_ImpresionMaestra] ('" & ws.Name & "')"
...
Attribute GetAbsoluteUsedRange.VB_ProcData.VB_Invoke_Func = " \n21"
    ...
    If ws.PageSetup.PrintArea = "" Then
        LogWarning MODULE_NAME, "[GetAbsoluteUsedRange] ('" & ws.Name & "') No hay área de impresión definida en la hoja '" & ws.Name & "'."
    End If
...
Attribute ConfigurarSaltosDePagina.VB_ProcData.VB_Invoke_Func = " \n0"
    ...
    If npb = 0 Then
        LogWarning MODULE_NAME, "[ConfigurarSaltosDePagina] No hay saltos automáticos en '" & ws.Name & "'. ¿Rango imprimible vacío?"
        GoTo CleanUp
    End If
    
    LogInfo MODULE_NAME, "[ConfigurarSaltosDePagina] AJUSTE DE SALTOS (basado en " & npb & " saltos automáticos) ==="
    ...
    For pagina = 1 To npb + 1  ' +1 para incluir la última página (después del último salto)
        ...
        If mejorFilaMarcador > 0 And mejorDistancia < 15 Then  ' umbral: <15 pts - 0.2 cm
            ...
            LogInfo MODULE_NAME, "[ConfigurarSaltosDePagina] Página " & pagina & ": salto ajustado a fila " & mejorFilaMarcador & _
                        " (f=" & Round(mejorFactorF, 2) & ", d=" & Round(mejorDistancia, 1) & " pts)"
        Else
            LogInfo MODULE_NAME, "[ConfigurarSaltosDePagina] Página " & pagina & ": sin marcador óptimo (rango " & filaInicio & "-" & filaFin & ")"
        End If
    Next pagina
...
CleanUp:
    Application.ScreenUpdating = prevScreenUpdating
    If Err.Number <> 0 Then
        LogCurrentError MODULE_NAME, "[ConfigurarSaltosDePagina]"
    Else
        LogInfo MODULE_NAME, "[ConfigurarSaltosDePagina] finalizado. Nuevos saltos: " & ws.HPageBreaks.Count
    End If
End Sub
...
Attribute ConfigurarSaltosDePagina_v2.VB_ProcData.VB_Invoke_Func = " \n0"
    ...
    Do While filaInicioPagina <= ultimaFila
        ...
        If filaMarcador > 0 Then
            ...
            LogInfo MODULE_NAME, "[ConfigurarSaltosDePagina_v2] Salto insertado antes de fila " & filaMarcador & " (posición de marcador)"
        Else
            ...
            Else
                ...
                LogWarning MODULE_NAME, "[ConfigurarSaltosDePagina_v2] Sin saltos automáticos ni marcadores: avance forzado a fila " & filaInicioPagina
            End If
        End If
    ...
    LogInfo MODULE_NAME, "[ConfigurarSaltosDePagina_v2] " & npb & " saltos en '" & ws.Name & "'"
    
CleanUp:
    Application.ScreenUpdating = prevScreenUpdating
    If Err.Number <> 0 Then
        LogCurrentError MODULE_NAME, "[ConfigurarSaltosDePagina_v2]"
    End If
End Sub
...
Attribute CompactarAreaDeImpresion.VB_ProcData.VB_Invoke_Func = " \n0"
    ...
    If ws.PageSetup.PrintArea = "" Then
        LogWarning MODULE_NAME, "[CompactarAreaDeImpresion] no hay área de impresión en '" & ws.Name & "'."
        Exit Sub
    End If
    ...
    If Not hayMultiples Then
        LogInfo MODULE_NAME, "[CompactarAreaDeImpresion] solo un rango. Nada que hacer."
        estado.EsValido = True  ' aún así, marcamos como válido (restaurable)
        Exit Sub
    End If
    
    LogInfo MODULE_NAME, "[CompactarAreaDeImpresion] " & (UBound(printAreas) + 1) & " rangos en '" & ws.Name & "' ==="
    ...
    LogInfo MODULE_NAME, "[CompactarAreaDeImpresion] Filas ocultas (huecos): " & filasOcultas
    LogInfo MODULE_NAME, "[CompactarAreaDeImpresion] Columnas ocultas (huecos): " & colsOcultas
    LogInfo MODULE_NAME, "[CompactarAreaDeImpresion] Área unificada: " & unifiedRange.Address
    LogInfo MODULE_NAME, "[CompactarAreaDeImpresion] finalizado."
    
    Exit Sub

ErrorHandler:
    LogCurrentError MODULE_NAME, "[CompactarAreaDeImpresion]"
    estado.EsValido = False
End Sub
...
Attribute RestaurarAreaDeImpresion.VB_ProcData.VB_Invoke_Func = " \n0"
    ...
    If Not estado.EsValido Then
        LogWarning MODULE_NAME, "[RestaurarAreaDeImpresion] estado no válido. Nada restaurado."
        Exit Sub
    End If
    ...
    LogInfo MODULE_NAME, "[CompactarAreaDeImpresion] Columnas restauradas a visible: " & colsRestauradas
    
    Exit Sub

ErrorHandler:
    LogCurrentError MODULE_NAME, "[RestaurarAreaDeImpresion]"
End Sub
