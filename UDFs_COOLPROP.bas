...
'              disjuntos. Cada Ã¡rea del rango es tratada como una tabla independiente con
'              encabezados ("nombre/gas" y "%/percentage") en la primera fila.
'@Scope:
'@ArgumentDescriptions: rango: Rango contiguo o disjunto, donde cada Ã¡rea contiene columnas vÃ¡lidas
...
Attribute ConstruirCadenaCoolPropDesdeTabla.VB_Description = "[UDFs_COOLPROP] Construye una cadena HEOS::.. para CoolProp procesando uno o varios rangos disjuntos. Cada Ã¡rea del rango es tratada como una tabla independiente con encabezados (""nombre/gas"" y ""%/percentage"") en la primera fila. Aplica a: Cells Range"
Attribute ConstruirCadenaCoolPropDesdeTabla.VB_ProcData.VB_Invoke_Func = " \n21"
    ...
    ' Rangos colectivos que contendrÃ¡n TODAS las columnas de Nombres/Porcentajes
    ...
    ' --- Fase 1: Identificar todas las columnas relevantes en todas las Ã¡reas ---
    For Each area In rango.Areas
        For Each cabecera In area.Rows(1).Cells
            Select Case LCase(Trim(cabecera.Value))
            Case "nombre", "gas"
                ' AÃ±adir la columna completa al rango global de nombres
                ...
                End If
            Case "%", "percentage"
                ' AÃ±adir la columna completa al rango global de porcentajes
    ...
    If celdasNombresValidas Is Nothing Or celdasPorcentajesValidas Is Nothing Then
        ConstruirCadenaCoolPropDesdeTabla = "Error: No hay datos vÃ¡lidos en las Ã¡reas seleccionadas."
        Exit Function
    End If
    
    ' Asumimos que la lista de celdas de nombres y porcentajes son del mismo tamaÃ±o
    ' y estÃ¡n en el mismo orden (e.g., A8, A9, A10; F8, F9, F10).
    ' Este bucle itera sobre las celdas vÃ¡lidas (excepto la fila 1, que son cabeceras)
    
    Dim c As Long
    ' Usamos arrays para acceder por Ã­ndice, ya que collections no lo permiten fÃ¡cilmente con Rangos disjuntos
    ...
    If UBound(arrNombres) <> UBound(arrPorcentajes) Then
        ConstruirCadenaCoolPropDesdeTabla = "Error: Desajuste en el nÃºmero de valores."
        Exit Function
    End If

    ' Iterar sobre los arrays emparejados (empezando por el Ã­ndice 1, asumiendo que la fila 1 es cabecera y ya la hemos ignorado)
    ' Nota: La funciÃ³n CellsToArray maneja la omisiÃ³n de la primera fila.
...
' --- FunciÃ³n de ayuda para convertir un rango disjunto en un array plano ---
Private Function CellsToArray(inputRange As Range) As Variant()
    ...
    For Each area In inputRange.Areas
        ' Omitimos la primera fila de cada Ã¡rea, ya que son cabeceras
