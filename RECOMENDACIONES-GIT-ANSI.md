# Recomendaciones de configuración Git para proyectos VBA con ficheros ANSI

> **Contexto de este proyecto:**
> Los ficheros VBA (`.cls`, `.bas`, `.frm`) se editan en **dos entornos distintos**:
...
- El editor VBA de Office lee y escribe ANSI directamente: ninguna conversión manual es necesaria.
...
python3 -c "
import glob
for f in glob.glob('*.cls')+glob.glob('*.bas')+glob.glob('*.frm'):
...
echo "Clon Linux listo. Todos los ficheros VBA están en UTF-8."
```
...
:: o para todos los ficheros VBA:
git checkout -- "*.cls" "*.bas" "*.frm"
```

---

## 6. Regla de oro: nunca editar ficheros VBA fuera de Git

Cualquier modificación de ficheros `.cls`, `.bas`, `.frm` que **no pase por Git**
(guardar desde un editor externo directamente en la carpeta espejo) puede romper
la cadena de conversión. La única excepción es el editor VBA de Office, que siempre
